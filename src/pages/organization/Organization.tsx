...
function Organization() {
  const [organizationSettings, setOrganizationSettings] =
    useState<OrganizationSettings>(defaultOrganizationSettings);
  ...
  function removeOrganizationExpertise(
    expertiseToRemove: string
  ) {
    setOrganizationSettings((previousSettings) => ({
      ...previousSettings,
      expertise: previousSettings.expertise.filter(
        (item) => item !== expertiseToRemove
      ),
    }));
  }
        ...
        {!isEditing ? (
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => {
                setOrganizationSettings(previewSettings);
                setIsEditing(false);
              }}
            ...
            <button
              ...
              onClick={() => {
                setPreviewSettings(organizationSettings);
                setIsEditing(false);
                toast.success(
  ...
  <div className="mb-6">
    <div>
      ...
      <p className="mt-1 text-sm text-text-secondary">
        Update your organization details. These will be visible on employee
        cards.
      </p>
    </div>
  </div>
  ...
  <div className="flex flex-1 flex-col gap-4">
    <div className="grid gap-4 md:grid-cols-[118px_1fr] md:items-start">
      <label className="pt-2 text-sm text-text-secondary">
        Company Logo
      </label>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="space-y-1">
          <p className="text-sm font-semibold text-text">
            {organizationSettings.logoName}
          </p>

          <p className="text-xs text-text-secondary">
            {organizationSettings.logoMeta}
          </p>
    ...
    {[
      ...
      const value =
        organizationSettings[field as keyof OrganizationSettings] as string;

      return (
        <div
          key={label}
          className="grid gap-4 md:grid-cols-[118px_1fr] md:items-start"
        >
          <label className="pt-2 text-sm text-text-secondary">
            {label}
          </label>
            ...
            <textarea
              ...
              onChange={(event) =>
                handleOrganizationFieldChange(
                  field as keyof OrganizationSettings,
                  event.target.value
                )
              }
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {organizationSettings.expertise.map((item) => (
            <span
              key={item}
              className="inline-flex items-center gap-1 rounded-sm bg-primary px-2 py-0.5 text-[11px] font-semibold text-white shadow-sm"
            >
              {item}
              ...
              <button
                ...
                disabled={!isEditing}
                onClick={() =>
                  removeOrganizationExpertise(item)
                }
        ...
        <div className="mt-2 flex items-center gap-2 rounded-md border border-border bg-white px-3 py-2.5">
          ...
          <button
            type="button"
            disabled={!isEditing}
            onClick={addOrganizationExpertise}
            className="flex h-5 w-5 items-center justify-center rounded-full border border-primary text-primary transition hover:bg-primary hover:text-white"
          >
...
<div className="mt-8">
  <div className="mb-4">
    ...
    <p className="text-sm text-text-secondary mt-1">
      Preview updates after clicking Save.
    </p>
  </div>

  <FlipCardContainer organization={previewSettings} />
</div>
