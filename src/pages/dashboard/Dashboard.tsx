        ...
        <AdminShell>
            <div className="mx-auto max-w-[1320px] space-y-6">
                    ...
                    {statCards.map((card) => {
                        ...
                        return (
                            <button
                                ...
                                className={`card p-5 text-left transition hover:shadow-hover ${activeTab === card.key ? "ring-2 ring-primary/20" : "hover:-translate-y-0.5"}`}
                            >
                                <div className="flex items-start justify-between gap-4">
                                    <div>
                                        <p className="text-sm text-text-secondary">{card.label}</p>
                                        <p className="mt-2 text-4xl font-semibold text-primary">{card.value}</p>
                ...
                <section className="card overflow-hidden">
                    <div className="border-b border-border-light px-5 pt-5 sm:px-6">
                        ...
                        <div className="mt-5 flex flex-wrap items-center gap-3 pb-5">
                            <div className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl border border-border bg-surface-container px-4 py-3">
                            ...
                            <button
                                type="button"
                                className="inline-flex items-center gap-2 rounded-2xl border border-border bg-surface px-4 py-3 text-sm font-medium text-text-secondary transition hover:bg-surface-container"
                            >
                    ...
                    <div className="p-3 sm:p-4">
                        <div className="space-y-2">
                            {visibleRows.map((row, index) => (
                                <div key={`${row.id}-${row.name}`} className="flex flex-wrap items-center gap-4 rounded-2xl px-3 py-4 transition hover:bg-surface-container">
